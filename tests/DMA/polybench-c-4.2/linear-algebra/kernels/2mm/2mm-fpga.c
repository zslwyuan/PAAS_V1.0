...
int main(int argc, char** argv)
{
  ...
  int *C=indata+(NI*NJ+NI*NK+NK*NJ);
  int *D=indata+(NI*NJ+NI*NK+NK*NJ+NJ*NL);
	p0 =(unsigned long long *) new((unsigned long long *)0xc0000000) unsigned long long[12];//Contro Port
  ...
  for (pp=0;pp<128*1024/4;pp++)
	...
	p0[6]=0;p0[6]=0;p0[6]=1;
  /* Start timer. */
  polybench_start_instruments;
	printf("begin:%llu end:%llu\n",(unsigned long long)&indata[0],(unsigned long long)&indata[NI*NJ+NI*NK+NK*NJ+NJ*NL+NI*NL+2]);
  printf("%d %d %d %d\n",indata[1],indata[1001],indata[2222],indata[3211]);
  /* Run kernel. */

 // kernel_2mm (indata);
  while(p0[6]);
  while(!p0[10]);//wait until DMA finish
