...
long long indata[NI*NJ+NI*NK+NK*NJ+NJ*NL+NI*NL+2];
...
{
  int i, j;
  long long *tmp=indata;
  long long *A=indata+(NI*NJ);
  long long *B=indata+(NI*NJ+NI*NK);
  long long *C=indata+(NI*NJ+NI*NK+NK*NJ);
  long long *D=indata+(NI*NJ+NI*NK+NK*NJ+NJ*NL);
  long long* alpha=indata+NI*NJ+NI*NK+NK*NJ+NJ*NL+NI*NL;
  long long* beta=indata+NI*NJ+NI*NK+NK*NJ+NJ*NL+NI*NL+1;
  *alpha = 2;
  *beta = 3;
  for (i = 0; i < ni; i++)
    for (j = 0; j < nk; j++)
...
//static
void kernel_2mm(long long indata[NI*NJ+NI*NK+NK*NJ+NJ*NL+NI*NL+2])
{
  int i, j, k;
